...
int auth(const char* username, const char* password, char* session_id, int* xor) {
    ...
    // step0: connect server
    // oh shit, the socket should be closed everytime when failed.
    int socket_fd = connect_server(SOCK_STREAM);
    if (socket_fd < 0) {
        return -1;
    }
...
int keep_alive(const char* user, const char* session_id, int xor_num) {
    ...
    int socket_fd = connect_server(SOCK_DGRAM);
    if (socket_fd < 0) {
        return -1;
    }
...
int main(int argc, char const *argv[])
{
    ...
    if (auth(user, pass, session_id, &xor_num) == 0) {
        printf("pass\n");
    }
    else {
        fprintf(stderr, "failed to pass zhunru\n");
        return -1;
    }
    ...
    do {
        sleep(60);
        keep_alive(user, session_id, xor_num);
